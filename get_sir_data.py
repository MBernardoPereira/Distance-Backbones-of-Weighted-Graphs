...
def produce_info_file(net_name, net_directory, sparsifier_name, type_of_subgraphs, n_real, n_sims, list_net_sizes, list_percentages, seed_nodes, beta, appendix, fraction_connected_subgraphs=None):
    """
    Documentation
    """
    print(f'Producing info file | Network {net_name} | Sparsifier {sparsifier_name} |  Subgraphs {type_of_subgraphs} ...')
    f = open(net_directory+f'/sir/info_{net_name}_{sparsifier_name}_{type_of_subgraphs}{appendix}.txt', 'w')
    f.write('network:'+net_name+'\n')
    f.write('ep_model:sir\n')
...
def produce_data_file(net_name, net_directory, sparsifier_name, type_of_subgraphs, subgraphs, seed_nodes, nr_simulations, beta, appendix):
    """
    Documentation
    """
    ...
    for nnr in range(nr_net_sizes):
        ...
        get_data_function = sirf.get_data_from_subgraph
        #sirf.get_data_from_subgraph(subgraph, seed_nodes, number_of_spreadings, beta, data_retrieved)
        inputs = zip(current_subgraphs, 
                    ...
                    [beta for i in range(nr_current_subgraphs)], 
                    ['r_inf_values' for i in range(nr_current_subgraphs)])
    ...
    f = open(net_directory+f'/sir/data_{net_name}_{sparsifier_name}_{type_of_subgraphs}{appendix}.txt', 'w')
...
if __name__ == '__main__':   
    ...
    for net_name in ['frenchHSNet']:#,'exhibitNet', 'workplaceNet']:
        for beta in [3,4,5]:
            ...
            appendix='_'+net_name_to_info_code[net_name][beta]
            ...
            #Beta parameter of the SIR model (Taking in mind that Gamma=1, so Beta is the R0 for this model)
            #beta=3
            ...
            produce_info_file(net_name, net_directory, 'threshold_proximity', 'random', n_real, n_sims, pb_subgraphs_sizes, pb_subgraphs_sizes_percentages, seed_nodes, beta, '_pb'+appendix, fraction_connected_subgraphs=trs_fcs)
            produce_data_file(net_name, net_directory, 'threshold_proximity', 'random', tr_subgraphs, seed_nodes, n_sims, beta, '_pb'+appendix)

            #THRESHOLD PROXIMITY RANDOM SUBGRAPHS
            tr_subgraphs = sgf.threshold_random_subgraphs(m_net, mb_subgraphs_sizes, n=n_real)
            trs_fcs = sgf.get_fraction_connected_subgraphs(tr_subgraphs)
            produce_info_file(net_name, net_directory, 'threshold_proximity', 'random', n_real, n_sims, mb_subgraphs_sizes, mb_subgraphs_sizes_percentages, seed_nodes, beta, '_mb'+appendix, fraction_connected_subgraphs=trs_fcs)
            produce_data_file(net_name, net_directory, 'threshold_proximity', 'random', tr_subgraphs, seed_nodes, n_sims, beta, '_mb'+appendix)

            #THRESHOLD PROXIMITY RANDOM SUBGRAPHS
            tr_subgraphs = sgf.threshold_random_subgraphs(e_net, eb_subgraphs_sizes, n=n_real)
            trs_fcs = sgf.get_fraction_connected_subgraphs(tr_subgraphs)
            produce_info_file(net_name, net_directory, 'threshold_proximity', 'random', n_real, n_sims, eb_subgraphs_sizes, eb_subgraphs_sizes_percentages, seed_nodes, beta, '_eb'+appendix, fraction_connected_subgraphs=trs_fcs)
            produce_data_file(net_name, net_directory, 'threshold_proximity', 'random', tr_subgraphs, seed_nodes, n_sims, beta, '_eb'+appendix)

            #THRESHOLD PROXIMITY RANDOM SUBGRAPHS
            tr_subgraphs = sgf.threshold_random_subgraphs(um_net, umb_subgraphs_sizes, n=n_real)
            trs_fcs = sgf.get_fraction_connected_subgraphs(tr_subgraphs)
            produce_info_file(net_name, net_directory, 'threshold_proximity', 'random', n_real, n_sims, umb_subgraphs_sizes, umb_subgraphs_sizes_percentages, seed_nodes, beta, '_umb'+appendix, fraction_connected_subgraphs=trs_fcs)
            produce_data_file(net_name, net_directory, 'threshold_proximity', 'random', tr_subgraphs, seed_nodes, n_sims, beta, '_umb'+appendix)
            ...
            produce_info_file(net_name, net_directory, 'random_subgraph', 'random', n_real, n_sims, umb_subgraphs_sizes, umb_subgraphs_sizes_percentages, seed_nodes, beta, appendix, fraction_connected_subgraphs=rrs_fcs)
            produce_data_file(net_name, net_directory, 'random_subgraph', 'random', rr_subgraphs, seed_nodes, n_sims, beta, appendix)
            ...
            produce_info_file(net_name, net_directory, 'threshold_proximity', 'threshold', n_real, n_sims, umb_subgraphs_sizes, umb_subgraphs_sizes_percentages, seed_nodes, beta, appendix, fraction_connected_subgraphs=tps_fcs)
            produce_data_file(net_name, net_directory,'threshold_proximity', 'threshold', tp_subgraphs, seed_nodes, n_sims, beta, appendix)

            #ULTRAMETRIC BACKBONE RANDOM SUBGRAPHS
            umbr_subgraphs = sgf.backbone_random_subgraphs(um_net, umb_subgraphs_sizes, n=n_real)
            produce_info_file(net_name, net_directory, 'ultrametric_backbone', 'random', n_real, n_sims, umb_subgraphs_sizes, umb_subgraphs_sizes_percentages, seed_nodes, beta, appendix)
            produce_data_file(net_name, net_directory, 'ultrametric_backbone', 'random', umbr_subgraphs, seed_nodes, n_sims, beta, appendix)

            #ULTRAMETRIC BACKBONE DISTORTION SUBGRAPHS
            umbtd_subgraphs = sgf.threshold_distortion_subgraphs(um_net, umb_subgraphs_sizes)
            produce_info_file(net_name, net_directory, 'ultrametric_backbone', 'distortion', 1, n_sims, umb_subgraphs_sizes, umb_subgraphs_sizes_percentages, seed_nodes, beta, appendix)
            produce_data_file(net_name, net_directory, 'ultrametric_backbone', 'distortion', umbtd_subgraphs, seed_nodes, n_sims, beta, appendix)

            #EUCLIDEAN BACKBONE RANDOM SUBGRAPHS
            ebr_subgraphs = sgf.backbone_random_subgraphs(e_net, eb_subgraphs_sizes, n=n_real)
            produce_info_file(net_name, net_directory, 'euclidean_backbone', 'random', n_real, n_sims, eb_subgraphs_sizes, eb_subgraphs_sizes_percentages, seed_nodes, beta, appendix)
            produce_data_file(net_name, net_directory, 'euclidean_backbone', 'random', ebr_subgraphs, seed_nodes, n_sims, beta, appendix)

            #EUCLIDEAN BACKBONE DISTORTION SUBGRAPHS
            ebtd_subgraphs = sgf.threshold_distortion_subgraphs(e_net, eb_subgraphs_sizes)
            produce_info_file(net_name, net_directory, 'euclidean_backbone', 'distortion', 1, n_sims, eb_subgraphs_sizes, eb_subgraphs_sizes_percentages, seed_nodes, beta, appendix)
            produce_data_file(net_name, net_directory, 'euclidean_backbone', 'distortion', ebtd_subgraphs, seed_nodes, n_sims, beta, appendix)

            #METRIC BACKBONE RANDOM SUBGRAPHS
            mbr_subgraphs = sgf.backbone_random_subgraphs(m_net, mb_subgraphs_sizes, n=n_real)
            produce_info_file(net_name, net_directory, 'metric_backbone', 'random', n_real, n_sims, mb_subgraphs_sizes, mb_subgraphs_sizes_percentages, seed_nodes, beta, appendix)
            produce_data_file(net_name, net_directory, 'metric_backbone', 'random', mbr_subgraphs, seed_nodes, n_sims, beta, appendix)

            #METRIC BACKBONE DISTORTION SUBGRAPHS
            mbtd_subgraphs = sgf.threshold_distortion_subgraphs(m_net, mb_subgraphs_sizes)
            produce_info_file(net_name, net_directory, 'metric_backbone', 'distortion', 1, n_sims, mb_subgraphs_sizes, mb_subgraphs_sizes_percentages, seed_nodes, beta, appendix)
            produce_data_file(net_name, net_directory, 'metric_backbone', 'distortion', mbtd_subgraphs, seed_nodes, n_sims, beta, appendix)

            #PRODUCT BACKBONE RANDOM SUBGRAPHS
            pbr_subgraphs = sgf.backbone_random_subgraphs(p_net, pb_subgraphs_sizes, n=n_real)
            produce_info_file(net_name, net_directory, 'product_backbone', 'random', n_real, n_sims, pb_subgraphs_sizes, pb_subgraphs_sizes_percentages, seed_nodes, beta, appendix)
            produce_data_file(net_name, net_directory, 'product_backbone', 'random', pbr_subgraphs, seed_nodes, n_sims, beta, appendix)

            #PRODUCT BACKBONE DISTORTION SUBGRAPHS
            pbtd_subgraphs = sgf.threshold_distortion_subgraphs(p_net, pb_subgraphs_sizes)
            produce_info_file(net_name, net_directory, 'product_backbone', 'distortion', 1, n_sims, pb_subgraphs_sizes, pb_subgraphs_sizes_percentages, seed_nodes, beta, appendix)
            produce_data_file(net_name, net_directory, 'product_backbone', 'distortion', pbtd_subgraphs, seed_nodes, n_sims, beta, appendix)
